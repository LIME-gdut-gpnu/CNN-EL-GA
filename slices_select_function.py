...
from svm_sklearn import use_svm
...
class CNN_classifier(object):
    ...
    #     # with tf.variable_scope("CNN2d"+str(self.model_idx),reuse=tf.AUTO_REUSE) as scope:
    #     with tf.variable_scope("CNN2d", reuse=tf.AUTO_REUSE) as scope:
    #         c1 = self.relu(self.conv2d(inputs, self.fliter_size1, self.stride1, name='conv1'))  # 卷积层1
    #         p1 = self.max_pool(c1, self.pool_size1, self.pool_stride1)  # 池化层2
    #
    #         c2 = self.relu(self.conv2d(p1, self.fliter_size2, self.stride2, name='conv2'))  # 卷积层2
    #         p2 = self.max_pool(c2, self.pool_size2, self.pool_stride2)  # 池化层2
    #
    #         c3 = self.relu(self.conv2d(p2, self.fliter_size3, self.stride3, name='conv3'))  # 卷积层3
    #         p3 = self.max_pool(c3, self.pool_size3, self.pool_stride3)  # 池化层3
    ...
    def CAE(self, inputs, reuse=False):
        ...
        with tf.variable_scope("CNN2d", reuse=tf.AUTO_REUSE) as scope:
            ...
            de_conv1 = self.relu(
                self.conv2d(upsample1, fliter_size=[3, 3, 1024, 512], stride=[1, 1, 1, 1], name='de_conv1'))
            #
            # upsample2 = tf.image.resize_images(encoder, size=(6, 6), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            de_conv2 = self.relu(
                self.conv2d(de_conv1, fliter_size=[3, 3, 512, 256], stride=[1, 1, 1, 1], name='de_conv2'))

            de_conv3 = self.relu(
                self.conv2d(de_conv2, fliter_size=[3, 3, 256, 128], stride=[1, 1, 1, 1], name='de_conv3'))

            upsample4 = tf.image.resize_images(de_conv3, size=(17, 17), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            de_conv4 = self.relu(
                self.conv2d(upsample4, fliter_size=[3, 3, 128, 64], stride=[1, 1, 1, 1], name='de_conv4'))

            upsample5 = tf.image.resize_images(de_conv4, size=(49, 49), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            de_conv5 = self.relu(
                self.conv2d(upsample5, fliter_size=[3, 3, 64, 32], stride=[1, 1, 1, 1], name='de_conv5'))

            upsample6 = tf.image.resize_images(de_conv5, size=(145, 145), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            de_conv6 = self.relu(
                self.conv2d(upsample6, fliter_size=[3, 3, 32, 1], stride=[1, 1, 1, 1], name='de_conv6'))
            ...
            # # de_conv1(200,6,6,512)
            # upsample1 = tf.image.resize_images(encoded, size=(6, 6), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            # de_conv1 = self.relu(self.conv2d(upsample1, [1,1,1024,512], self.stride6, name='de_conv1'))
            # # de_conv2(200,6,6,256)
            # de_conv2 = self.relu(self.conv2d(de_conv1, [1, 1, 512, 256], self.stride6, name='de_conv2'))
            # # de_conv3(200,6,6,128)
            # de_conv3 = self.relu(self.conv2d(de_conv2, [1, 1, 256, 128], self.stride6, name='de_conv3'))
            #
            # # de_conv4(200,17,17,64)
            # upsample2 = tf.image.resize_images(de_conv3, size=(17, 17), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            # de_conv4 = self.relu(self.conv2d(upsample2, [3, 3, 128, 64], self.stride3, name='de_conv4'))
            #
            # # de_conv5(200,49,49,32)
            # upsample3 = tf.image.resize_images(de_conv4, size=(49, 49), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            # de_conv5 = self.relu(self.conv2d(upsample3, [3, 3, 64, 32], self.stride3, name='de_conv5'))
            #
            # # logits(200,145,145,1)
            # upsample4 = tf.image.resize_images(de_conv5, size=(145, 145), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            # decoded_logits = self.conv2d(upsample4, [3, 3, 32, 1], self.stride3, name='de_conv6')

            # de_conv4(200,17,17,64)
            # upsample2 = tf.image.resize_images(p3, size=(17, 17), method=tf.image.ResizeMethod.NEAREST_NEIGHBOR)
            #########################
            # de_kernel1 = tf.constant(1.0, shape=[3, 3, 64, 128])
            # de_kernel2 = tf.constant(1.0, shape=[3, 3, 32, 64])
            # de_kernel3 = tf.constant(1.0, shape=[3, 3, 1, 32])
            # upsample2 = tf.nn.conv2d_transpose(p3, de_kernel1, output_shape=[200, 17, 17, 64], strides=[1, 3, 3, 1],
            #                                    padding="SAME")
            #
            # # de_conv5(200,49,49,32)
            # upsample3 = tf.nn.conv2d_transpose(upsample2, de_kernel2, output_shape=[200, 49, 49, 32],
            #                                    strides=[1, 3, 3, 1], padding="SAME")
            #
            # # logits(200,145,145,1)
            # upsample4 = tf.nn.conv2d_transpose(upsample3, de_kernel3, output_shape=[200, 145, 145, 1],
            #                                    strides=[1, 3, 3, 1], padding="SAME")
            # decoded_logits = upsample4
            # decoded = tf.nn.sigmoid(decoded_logits)
            ###################################
    ...
    def build_model(self):
        ...
        with self.g1.as_default():
            ...
            #     self.CNN,self.CNN_logits,self.fc1=self.CNN2d(self.inputs,self.keep_prob)
            # else:
            #     self.CNN, self.CNN_logits,self.fc1= self.CNN2d_test(self.inputs, self.keep_prob)        #test模型BN层的trainning设置为False

            # 计算损失
            self.loss = tf.nn.softmax_cross_entropy_with_logits(logits=self.CNN_logits, labels=self.labels)

            # 增加L2正则化项
            # regularizer1 = tf.contrib.layers.l1_regularizer(0.000001)#0.00001
            # regularizer2 = tf.contrib.layers.l2_regularizer(0.001)
            # tmp=tf.trainable_variables()
            # #
            # weights_list1 = [g for g in tmp if ('w' in g.name and 'conv' in g.name)]
            # self.l1_loss=tf.contrib.layers.apply_regularization(regularizer1, weights_list=weights_list1)
            #
            # weights_list2 = [g for g in tmp if ('w' in g.name and 'fc' in g.name)]
            # self.l2_loss = tf.contrib.layers.apply_regularization(regularizer2, weights_list=weights_list2)
            # #
            # # self.loss=self.loss+self.l1_loss+self.l2_loss
            # self.loss = self.loss  + self.l2_loss

            # self.loss = -tf.reduce_sum(self.labels * tf.log(self.CNN))  # 自定义交叉熵函数
            # 更新batchnormal的参数
                        ...
                        try:
                            ...
                            if counter % 10 == 0:
                                self.save_model(sess, counter,
                                                self.pretrain_checkpoint_dir)  # 这里有个参数默认值max_to_keep=5，表示保留最新的5个检查点

                            if counter % 100 == 0:
                                # 保存重建的图像
                                imgs = self.decoded.eval({self.inputs: batch_images, })
                                img = imgs[0, :, :, 0]
                                full_save_name = os.path.join(self.pretrain_checkpoint_dir, 'recon_imgs',
                                                              'img_' + str(counter) + '.jpg')
                                self.save_img(full_save_name, img)
                                #
                                imgs2 = self.encoder.eval({self.inputs: batch_images, })
                                img2 = imgs2[0, :, :, 0]
                                full_save_name2 = os.path.join(self.pretrain_checkpoint_dir, 'feature_imgs',
                                                               'img_' + str(counter) + '.jpg')
                                self.save_img(full_save_name2, img2)

                                imgs3 = batch_images
                                img3 = imgs3[0, :, :, 0]
                                full_save_name3 = os.path.join(self.pretrain_checkpoint_dir, 'origin_imgs',
                                                               'img_' + str(counter) + '.jpg')
    ...
    def train(self):
        ...
        with self.g1.as_default():
            with tf.control_dependencies(self.update_ops):
                self.cnn_optim = tf.train.AdamOptimizer(self.learning_rate).minimize(loss=self.loss,
                                                                                     var_list=self.t_vars)  # 调用优化器优化
        ...
        with tf.Session(config=self.run_config, graph=self.g1) as sess:
            ...
            #         model_path = os.path.join(self.pretrain_checkpoint_dir, model_name)
            #         # model_file = tf.train.latest_checkpoint(self.checkpoint_dir)
            #         self.saver_cnn.restore(sess, model_path)
            #         print('successfully load model from: ' + model_path)
            #     else:
            #         print('No checkpoint file found')
            #         return
            ...
            for epoch in range(self.max_epoch):
                ...
                #     self.cnn_optim = tf.train.AdamOptimizer(learning_rate=learning_rate, beta1=self.momentum).minimize(self.loss)  # 调用优化器优化
                #     #这里新建了cnn_optim导致有些新的参数没初始化，需要检测出来进行初始化
                #     uninit_vars = []
                ...
                for idx in range(0, batch_idxs):
                    try:
                        ###调试信息##################################################################################
                        ##输出权值矩阵
                        # w1 = self.get_weight_bias("CNN2d/conv1/w:0")
                        # b1 = self.get_weight_bias("CNN2d/conv1/b:0")
                        # w2 = self.get_weight_bias("CNN2d/conv2/w:0")
                        # b2 = self.get_weight_bias("CNN2d/conv2/b:0")
                        # w3 = self.get_weight_bias("CNN2d/conv3/w:0")
                        # b3 = self.get_weight_bias("CNN2d/conv3/b:0")
                        # print("w1[:,:,0,0]:\n", w1[:, :, 0, 0])
                        # print("w2[:,:,0,0]:\n", w2[:, :, 0, 0])
                        # print("w3[:,:,0,0]:\n", w3[:, :, 0, 0])
                        # print("b1:\n", b1)
                        ...
                        # 运行优化器（设置dropout率）
                        self.cnn_optim.run(
                            feed_dict={self.inputs: batch_images, self.labels: batch_labels, self.keep_prob: 0.6})

                        # 计算损失（训练集带augment）
                        err_CNN = self.loss.eval(
                            {self.inputs: batch_images, self.labels: batch_labels, self.keep_prob: 1.0})
                        mean_err = np.mean(err_CNN)
                        print(self.thread_name + " Epoch: [%2d] [%4d/%4d] time: %4.4f, mean loss: %.8f" % (
                        epoch, idx, batch_idxs, time.time() - start_time, mean_err))
                        ...
                        if counter % 100 == 0:
                            ...
                            # 计算准确率
                            predict_labels = sess.run(self.CNN,
                                                      feed_dict={self.inputs: batch_images, self.labels: batch_labels,
                                                                 self.keep_prob: 1.0})
                            pred = np.argmax(predict_labels, 1)
                            real = np.argmax(batch_labels, 1)
                            # accuracy = sess.run(self.accuracy,feed_dict={self.inputs:batch_images,self.labels:batch_labels})
                            # print("Epoch: [%2d] [%4d/%4d] time: %4.4f, train accuracy: %.8f" % (epoch, idx, batch_idxs, time.time() - start_time, accuracy))
                            # file_names = [os.path.basename(batch_file) for batch_file in batch_files]  # 获取所有文件名
                            ...
                            print(
                                self.thread_name + " Epoch: [%2d] [%4d/%4d] time: %4.4f,train sensitivity: %.8f,specificity: %.8f,accuracy: %.8f,"
                                % (epoch, idx, batch_idxs, time.time() - start_time, SEN, SPC, ACC))

                            # 计算test data上的准确率################################################
                            # self.save_model(counter)            #先保存当前模型，再用test模型去加载
                            # acc_test,auc_test=CNN_model_test.test(sess_test)
                            # print("Epoch: [%2d] [%4d/%4d] time: %4.4f,test accuracy: %.8f,"
                            #       % (epoch, idx, batch_idxs, time.time() - start_time, acc_test))

                            # random.shuffle(data_test)  # 保证每次取的test data不确定
                            # test_batch = data[0:self.batch_size]  # 为了和训练时模型的placeholder保持一致

                            ##计算当前模型在训练集（不带augment）上的准确率#################
                            acc_train = sess.run(accuracy_train,
                                                 feed_dict={self.inputs_train: train_images,
                                                            self.labels_train: train_labels,
                                                            self.keep_prob: 1.0})
                            print(
                                "#############" + self.thread_name + " Epoch: [%2d] [%4d/%4d] time: %4.4f,train set accuracy:    %.8f"
                                % (epoch, idx, batch_idxs, time.time() - start_time, acc_train))

                            ###计算当前模型在验证集（509数据集之外的数据）上的准确率#################
                            # acc_test = sess.run(accuracy_test,
                            #                          feed_dict={self.inputs_test: test_images, self.labels_test: test_labels,
                            #                                     self.keep_prob: 1.0})
                            # print("#############"+self.thread_name+" Epoch: [%2d] [%4d/%4d] time: %4.4f,validate set accuracy:    %.8f"
                            #       % (epoch, idx, batch_idxs, time.time() - start_time, acc_test))

                            # 当前模型满足一定条件时，则备份该模型（这里设置的条件是当训练集准确率达到85%以上且验证集上准确率比上次好，则保留该模型）
                            # if acc_train>0.85 and acc_test >= best_acc_test:
                            #     best_acc_test=acc_test
                            #     print("#############"+self.thread_name+" Epoch: [%2d] [%4d/%4d] time: %4.4f,new best valid accuracy: 【%.8f】##############"
                            #           % (epoch, idx, batch_idxs, time.time() - start_time, acc_test))
                            #     self.backup_model(sess, counter)
    ...
    def test(self, sess, flag):
        ...
        with tf.Session(graph=self.g1) as sess:
            ...
            if flag == 0:
                data = np.loadtxt(self.valid_data_dir, dtype='str')
                data = sorted(data)
            else:
                # 根据文件路径获取测试数据
            ...
            for idx in range(0, batch_idxs):
                ...
                predict_prob, predict_logits = sess.run([self.CNN, self.CNN_logits],
                                                        feed_dict={self.inputs: batch_images, self.labels: batch_labels,
                                                                   self.keep_prob: 1.0})
...
def merge_different_axises_result(conf, file_names, real_labels, pred_lables, pred_probs):
    ...
    for fold in range(conf.fold_num):
        ...
        image_save_path = os.path.join(conf.base_report_dir,
                                       conf.classify_name + '_fold' + str(fold) + '_model_ROC.png')
...
def concat_path(conf, fold='', axis='', z=''):
    ...
    if fold == '':
        ...
        conf.train_data_dir = os.path.join(conf.data_dir, 'train_aug.txt')  # 训练图像所在的文件夹
        ...
        conf.pretrain_data_dir = os.path.join(conf.base_pretrain_data_dir, axis + str(z), 'smooth', 'all',
                                              'pretrain2.txt')
    else:
        conf.data_dir = os.path.join(conf.base_data_dir, axis + str(z), 'smooth')  # 训练图像所在的文件夹
        conf.train_data_dir = os.path.join(conf.data_dir, 'fold' + str(fold) + '_train_aug.txt')  # 训练图像所在的文件夹
        conf.test_data_dir = os.path.join(conf.data_dir, 'fold' + str(fold) + '_test.txt')  # 测试图像所在的文件夹
...
def main(_):
    ...
    conf.classify_name = classify_names[1]  # 当前训练哪种分类
        ...
        for p in range(len(slices_pos_all)):
            ...
            for z in slices_pos:
                ...
                if conf.use_process_pool:
                    ...
                    pool.apply_async(pretrain_thread,
                                     (run_config, conf_thread,))  # 加入到进程池，且各进程相互独立，可以同时训练pool_mult_num个模型
                else:
                    conf_thread = copy.deepcopy(conf)  # 保证每个进程独享一个conf参数
        ...
        for p in range(len(slices_pos_all)):
            ...
            if conf.use_cross_validation:
                ...
                for fold in range(conf.fold_num):
                    ...
                    report_txt_name = 'Test_Report_' + conf.classify_name + '_fold' + str(
                        fold) + '_axis_' + axis + '.txt'
                ...
                save_report_txt(conf.report_dir, report_txt_name, title)
                for z in slices_pos:
                    str2 = "Test on %s %s%d--ACC: %.8f, AUC: %.8f" % (
                    conf.classify_name, axis, z, te_ACC[te_idx], te_AUC[te_idx])
                    save_report_txt(conf.report_dir, report_txt_name, str2)
                    te_idx += 1
...
def getLabelMatrix(classify_flag, fold, slices_pos,slices_pos_xyz,run_flag,Data_flag, cv):
    # 这两句是对GPU的使用进行配置
    run_config = tf.ConfigProto()
    run_config.gpu_options.allow_growth = True
    ...
    # conf.slices_pos_all = slices_pos_all  # x,y,z轴上的切片位置
    conf.axises = axises  # 文件夹前缀
    # workdir = os.path.dirname(sys.argv[0])
    workdir = '/home/jlf/project/CNN_sMRI_2D'
    conf.base_data_dir = os.path.join(workdir, Data_flag, 'ADNI', 'slices', conf.classify_name)
    conf.base_valid_data_dir = os.path.join(workdir, 'Data', 'ADNI', 'slices', conf.classify_name_t)
    conf.base_checkpoint_dir = os.path.join(workdir, 'checkpoint', cv, conf.classify_name)
    conf.base_report_dir = os.path.join(workdir, 'report', conf.classify_name)
    conf.base_pretrain_checkpoint_dir = os.path.join(workdir, 'checkpoint')
    conf.base_pretrain_data_dir = os.path.join(workdir, 'Data', 'ADNI', 'slices', 'PreTrain')
    ...
    # 定义几个全局变量，用来保存测试时返回的结果
    result_labels = []
    for z in zip(slices_pos,slices_pos_xyz):
        aa = z[0]
        axis = z[1]
        concat_path(conf, fold=fold, axis=axis, z=aa)
        if run_flag == 0:
            # 确定batchsize.测试时，把所有的图像一次测完
            valid_data = np.loadtxt(conf.valid_data_dir, dtype='str')
            conf.batchsize = len(valid_data)
        else:
            test_data = np.loadtxt(conf.test_data_dir, dtype='str')
            conf.batchsize = len(test_data)

        # 此变量是因为多进程训练时，每次训练保存的模型参数前的前缀不同，所以要通过此变量保持一致
        conf.model_idx = 'f' + str(fold) + axis + str(aa)
        if not os.path.exists(conf.checkpoint_dir):
            print('Can not find checkpoint in checkpoint_dir:' + conf.checkpoint_dir)
        with tf.Session(config=run_config) as sess:
            CNN = CNN_classifier(sess, conf)
            pred, real = CNN.test(sess, run_flag)
            result_labels.append(pred)

    return result_labels,real
