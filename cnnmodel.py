            ...
            for epoch in range(self.max_epoch):
                ...
                for idx in range(0, batch_idxs):
                    ...
                    try:
                        ...
                        print(self.thread_name+" Epoch: [%2d] [%4d/%4d] time: %4.4f, mean loss: %.8f" % (epoch, idx, batch_idxs,time.time() - mid_start_time, mean_err))
...
def main(_):
    start_time = time.time()
    start_currtime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
    print('progressing start time:',start_currtime)
    ...
    # slices_pos_x = [72, 48, 76, 80, 74]#, 82, 38, 46, 30, 40]
    # slices_pos_y = [82,70,68,74,80]#, 94, 74, 62, 48, 56]
    # slices_pos_z = [36,34,38,32,42]#, 36, 68, 50, 46, 40]
    ...
    conf.train_flag = 1                     #执行什么操作（1：train，2：test）
    ...
    conf.pool_mult_num=2                   #进程池最大并发进程数
    conf.batchsize = 200                     #模型训练时训练数据的batchsize
    conf.classify_name=classify_names[0]    #当前训练哪种分类
    conf.classify_name_t = classify_names_t[0]
    ################################################################################
    ...
    conf.base_checkpoint_dir = os.path.join(workdir,'checkpoint_old',conf.classify_name)
    ...
    if conf.train_flag == 0:
        ...
        print('Test end')
    end_time = time.time()
    end_currtime = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime(time.time()))
    print('progress start time and end time:', start_currtime, end_currtime)
    print('Test end,spend_time:', (end_time - start_time) / 60)
...
def OneZero2Features(index, pop,slices_pos_list):
    ...
    # m, n, z = np.shape(dim_x)
    # slices_pos = np.reshape(dim_x, (m, z))
    return slices_pos


def cal_objvalue(conf,fold,axis,pop, slices_pos_list,run_config):
    px, py = np.shape(pop)
    ...
    for i in range(px):
        ...
        for z in slices_pos_selected:
            ...
            with tf.Session(config=run_config) as sess:
                if conf.use_best_one_model == True:
                    CNN = CNN_classifier(sess, conf)
                    predict_prob, pred, real, ACC, AUC, file_names = CNN.test(sess,0)
                else:
                    CNN = CNN_classifier(sess, conf)
                    predict_prob, pred, real, ACC, AUC, file_names = test_on_best_models(CNN, sess, conf)
                    # print('ACC:' + str(ACC))
        ...
        for prob in te_pred_prob:
            positive_scores = 0
            for k in range(len(slices_pos_selected)):
                positive_scores += prob[2 * k + 1]
            positive_scores_add.append(positive_scores / len(slices_pos_selected))
...
def select_slices_GA_FS(conf,fold,p,slices_pos_list,run_config):
    slices_len = len(slices_pos_list)
    ...
    for i in range(conf.iters):
        objvalue = cal_objvalue(conf,fold,axis,pop, slices_pos_list,run_config)
        fitvalue = objvalue
        ...
        pop = newpop_mutate
        fitvalue_current = cal_objvalue(conf,fold,axis,newpop_mutate,slices_pos_list,run_config)

        bestindividual, bestfit, bestIndex = best(newpop_mutate, fitvalue_current)
        if bestfit > best_auc:
            best_auc = copy.copy(bestfit)
...
# def get_ACC_AUC():
    ...
    slices_pos_best = []
    for p in range(len(slices_pos_all)):
        # slices_pos_list = conf.slices_pos_all[p]
        slices_pos = conf.slices_pos_all[p]
        axis = conf.axises[p]
        # print("%s zou\n" % axis)
        if conf.use_cross_validation:
            ...
            for fold in range(conf.fold_num):
                # print("fold:%d\n" % fold)
                # conf.chromlength=len(slices_pos_list)
                # slices_pos = select_slices_GA_FS(conf, fold, p, slices_pos_list, run_config)
                # slices_pos_fold.append(slices_pos.tolist())
                # 定义几个全局变量，用来保存测试时返回的结果
                te_pred_prob = [],
                te_pred = []
                te_ACC = []
            ...
            G_file_names = global_filenames
            G_real_lables = global_reallabels
            slices_pos_best.append(slices_pos_fold)
            # 打印并保存多折交叉验证的平均结果
            # save_report_txt函数里自带了print效果，调用save_report_txt保存信息的同时也会打印到屏幕上。后续如果节约时间可以把函数里的print注释掉
            # str1 = 'Mean ACC of all folds is :【' + str(mean_ACC) + '】'
            # str2 = 'Mean AUC of all folds is :【' + str(mean_AUC) + '】'
    ...
    Mean_ACC,Std_ACC,Mean_AUC,Std_AUC=merge_different_axises_result_and_return(conf, G_real_lables, G_pred_voted, G_pred_prob_voted)
    # print('Test end')
    return Mean_ACC,Std_ACC,Mean_AUC,Std_AUC
